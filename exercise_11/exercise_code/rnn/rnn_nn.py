...
class Embedding(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, padding_idx):
        ...
        self.weight = torch.normal(0,1,size=(self.num_embeddings, self.embedding_dim))
    ...
    def forward(self, inputs):
        ...
        # print(weight.shape)
        # print(inputs.shape)
        indices = inputs.flatten()

        # print(indices)

        embeddings = torch.index_select(weight, 0, indices)

        embeddings = embeddings.reshape(inputs.size(0), inputs.size(1), self.embedding_dim)

        embeddings.requires_grad = True

        # embeddings = embeddings.requires_grad(True)
