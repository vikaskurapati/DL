...
class DataLoader:
    ...
    def __iter__(self):
        ...
        usable_set = int(len(self.dataset)/self.batch_size) * self.batch_size

        if self.shuffle:
            index_iterator = np.random.permutation(len(self.dataset))
            if self.drop_last:
                index_iterator = index_iterator[0:usable_set]
            index_iterator = iter(index_iterator)  # define indices as iterator
        else:
            index_iterator = range(len(self.dataset))
            if self.drop_last:
                index_iterator = index_iterator[0:usable_set]
            index_iterator = iter(index_iterator)  # define indices as iterator
