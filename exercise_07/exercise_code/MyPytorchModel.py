...
class MyPytorchModel(pl.LightningModule):
    ...
    def __init__(self, hparams, input_size=3*32*32, hidden_size=512, num_classes=10):
        super().__init__()
        ...
        self.model = nn.Sequential(nn.Linear(input_size, hidden_size), nn.ReLU(), nn.Linear(hidden_size, num_classes), nn.Softmax())
    ...
    def forward(self, x):
        ...
        # x.shape = [self.hparams["batch_size"], 3, 32, 32] #-> flatten the image first
        # x.reshape(self.hparams["batch_size"], -1)
    ...
    def configure_optimizers(self):
        ...
        optim = torch.optim.Adam(self.model.parameters(), self.hparams["learning_rate"], weight_decay=0.1*self.hparams["learning_rate"] )
